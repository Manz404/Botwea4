...
const starts = async (kurr = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('MochinBotz Assistan', {
		font: 'Standard',
...
console.log(color('> HARI ','silver'), color(`${tampilHari}\n`,'mediumseagreen'))
	kurr.browserDescription = [ 'MochinBotz Asisstan', 'ubuntu', '3.0' ]
	...
	kurr.on('credentials-updated', () => {
		...
		info('2', 'MochinBotz Asisstan Loading...')
	})
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `MochinBotz-Assistan`, img,but = [{buttonId:`donasi`, 
			...
			try {
				...
				sendButImage(anu.jid, teks, `MochinBotz-Assistan`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `SAYONARA !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
